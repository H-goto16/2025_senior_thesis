...
\begin{itemize}
  ...
  \item 誤検出の傾向: 画面の反射や光沢、矩形に近い背景パターン、他機器（リモコン・充電器等）の類似外観で低スコアのFPが稀に発生。部分的に写るスマホ（遮蔽・極端なスケール変化）では検出の不安定化が起きやすい。
  \item 重複検出: 近接した複数候補が残る場合があり、1画素ズレの候補が同時に残るなど、NMS後も重複が残ることがある（本実装ではNMS閾値を明示設定していない）。
...
\begin{itemize}
  ...
  \item データ取り増し: 反射・逆光・部分遮蔽・遠距離など難条件の「負例（Hard Negative）」や多様な正例を収集して、次章の手動ラベリング＋短時間ファインチューニングに回すとFP/FNの双方が改善しやすい。
  ...
  \item 将来拡張: NMSしきい値や最大検出数のAPIパラメータ化、疑似負例（類似物体）を含む微調整レシピの提供、同義語正規化やクラス階層の導入で、FPに強い運用へ拡張可能。
\end{itemize}
...
\paragraph{査読付き国際学会}
\begin{enumerate}
    \item{Haruhiro Takahashi, Ryuto Ishibashi, Hayata Kaneko, and Lin Meng, ``Leftover Food Measurement using Deep Learning Based Semantic Segmentation," The 6th International Symposium on Advanced Technologies and Applications in the Internet of Things (ATAIT 2024), Aug. 2024. (in Kusatsu, Japan)}
    \item{Ishibashi, Ryuto, Haruhiro Takahashi, and Lin Meng. "ViT-Based Hybrid Segmentation for Leftover Food Detection." The 6th International Conference on Industrial Artificial Intelligence (IAI2024). Aug. 2024. (in Shenyang, China)}
\end{enumerate}

\paragraph{シンポジウム}
\begin{enumerate}
    \item{Haruhhiro Takhashi, ``Leftover Food Measurement using Segmentation and Detection", The 21th English Presentation Competition in Ritsumeikan University（EPCR2024）,Nov. 2024 (in Kusatsu, Japan)}
\end{enumerate}
